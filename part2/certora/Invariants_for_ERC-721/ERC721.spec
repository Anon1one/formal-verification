...
/**
 * The ghost variable _ownedByUser[user] stores how many tokens a user owns through the Sstore hook. The hook runs whenever the contract writes a new value to the 
 contract’s private storage mapping _owners[tokenId], and it observes both oldOwner and newOwner to determine how ownership has changed.
...
// address(0) has no authorized operator   
