...
invariant ethDepositsAreGreaterThanOrEqualToWethTotalSupply()
...
invariant noAccountBalanceShouldExceedsWethTotalSupply(env e1)
    balanceOf(e1.msg.sender) <= totalSupply()
    
    filtered {
        f -> f.selector != sig:transfer(address,uint256).selector && f.selector != sig:transferFrom(address, address, uint256).selector
    }
{
...
rule deposit_amountOfEthDepositedShouldBeEqualToWethRecieved_with_requireInvariant(env e){
    require e.msg.sender != currentContract;
    requireInvariant noAccountBalanceShouldExceedsWethTotalSupply(e);
    mathint ethBalanceBefore = nativeBalances[e.msg.sender];
    mathint wethBalanceBefore = balanceOf(e.msg.sender);
...
rule deposit_revert_with_requireInvariant(env e) {
    address caller = e.msg.sender;
    mathint ethDeposit = e.msg.value;
    requireInvariant noAccountBalanceShouldExceedsWethTotalSupply(e);
...
rule withdraw_decreasesTotalSupply_with_requireInvariant(env e) {
    uint256 amount;
    require e.msg.sender != currentContract;
