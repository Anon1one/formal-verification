...
*/
/**
 * One important thing to understand about a parametric rule is that it is designed to analyze the effect of just one function call at a time.
 * it means the prover is gonna select one function in the contract and gonna formally verify that function with our conditions
 * and it is gonna repeat this for all the functions in the contract
 *
 * a parametric rule only analyzes the effect of one external function call in isolation. 
 * It does not examine sequences of calls or complex interactions within a single transaction. 
*/
