...
rule nonOwnerCantCallIncrement() {
    env e;

    address owner = owner();
    increment@withrevert(e);

    assert e.msg.sender != owner => lastReverted; // if msg.sender is not owner then it should revert, we can simply use Implication operator
}